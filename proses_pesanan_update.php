 ...
 * ========================================================================================================
 * IKHSAN                1.0       08-12-2019             Pembuatan awal (create)
 */
...
if ( isset($_POST['nomor_pesanan']) && 
        ($_POST['nomor_pesanan'] != null || $_POST['nomor_pesanan'] != "") ) {
    $no_pesanan = $_POST['nomor_pesanan'];
} else {
    $no_pesanan     = getKodePesanan($tgl_pesanan);
}

tulisLog("proses_pesanan_update.php -> load..");
// tulisLog("proses_pesanan_update.php -> list_barang : ".$array_brg);
...
foreach ($array_brg as $value) {
	...
	$value_detail .= "("
		...
		. " upd_tms = NOW(),"
		. " upd_usr = '".$username."',"
		. " detail_pesanan_no_pesanan = '" . $no_pesanan . "',"
...
$value_detail = rtrim($value_detail, $sql_detail);
$value_detail = str_replace(',INSERT', '; INSERT', $value_detail);
$sql_detail .= $value_detail;
tulisLog('proses_pesanan_update.php -> string query detail_pesanan -> sql_detail : '.$sql_detail);
...
$sql_pesanan = "INSERT INTO pesanan "
			...
			. "";
tulisLog('proses_pesanan_update.php -> string query pesanan -> sql_pesanan : '.$sql_pesanan);
tulisLog('proses_pesanan_update.php -> insert ke pesanan -> sql_pesanan : '.$sql_pesanan);

if ( runUpdateInsertQuery($sql_pesanan) ) {

	tulisLog('proses_pesanan_update.php -> insert ke pesanan -> BERHASIL');
	$response = array('response_code' => '00', 'response_msg' => 'Pesanan berhasil disimpan');
	// echo json_encode($response);

	if (sizeof($array_brg) > 0) {

		$list_query = explode("; ", $sql_detail);
		$q_done = true;
		foreach ($list_query as $value) {
			tulisLog('proses_pesanan_update.php -> insert ke detail_pesanan -> value : '.$value);
			if ( runUpdateInsertQuery($value) ) {
				tulisLog('proses_pesanan_update.php -> insert ke detail_pesanan -> BERHASIL');
				$response = array('response_code' => '00', 'response_msg' => 'Detail pesanan berhasil disimpan');
			} else {
				tulisLog('proses_pesanan_update.php -> insert ke detail_pesanan -> GAGAL : '.mysqli_error($con));
	...
	tulisLog('proses_pesanan_update.php -> insert ke pesanan -> GAGAL : ' . mysqli_error($con));
...
tulisLog('proses_pesanan_update.php -> page close.');
