...
<table width="1000" border="0" cellspacing="0" cellpadding="3">
    ...
    <tr>
        <td class="main_rows" width="100"></td>
...
<br>
<table border="0.2" cellspacing="0" class="tabel">
    <thead>
        <tr>
...
foreach ($table_body as $value) {
    ...
    // buat $keterangan (BS/BP/BK) ada di kolom keterangan
    foreach ($list_table_detail as $vl) {
        if ($vl["detail_pesanan_keterangan"] !== "") {
            $nwarna = $vl["detail_pesanan_nama_warna"];
            $dkain = $vl["detail_pesanan_detail_kain"];
            if ($nwarna !== "") { $nwarna .= ", "; }
            if ($dkain !== "") { $dkain .= ", "; }

            $keterangan .= $nwarna . $dkain . $vl["detail_pesanan_ukuran"]." : ".$vl['detail_pesanan_keterangan'] . "<br>";
        }
    }
